...
public class BarFrame extends JFrame implements ChangeListener
{
   ...
   */
   public BarFrame(DataModel dataModel)
   {
      ...
      //Adding mouse click listener to update data point with a click
      addMouseListener(new MouseInputAdapter() {
         @Override
         public void mousePressed(MouseEvent e) {
            Point mousePoint = e.getPoint();
            int index = ((int) mousePoint.getY() / (barIcon.getIconHeight() / a.size())) - 1;
            index = index < 0 ? 0 : index;
            double value = (mousePoint.getX()/barIcon.getIconWidth()) * max ;
            dataModel.update(index, value);
